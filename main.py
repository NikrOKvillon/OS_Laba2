...
def create_signal_single_point(time_point, num_exp, num_cos, num_log, amp_exp, amp_cos, amp_log):
    ...
    exp_const = 1       # Константа для экспоненциального затухания
    freq_base = 2 * np.pi  # Базовая частота для косинусных членов
    phase_shift = 0     # Фазовый сдвиг для косинусных членов
    log_const = 1       # Константа для логарифмического члена
    k_base = 1          # Базовое значение для логарифмического члена
...
class SignalApp:
    def __init__(self, master):
        ...
        self.num_exp = 2          # Количество экспоненциальных членов
        self.num_cos = 1          # Количество косинусных членов
        self.num_log = 1          # Количество логарифмических членов
        self.amp_exp = [0.5, 0.5] # Амплитуды для экспоненциальных членов
        self.amp_cos = [1]        # Амплитуды для косинусных членов
        self.amp_log = [0.8]      # Амплитуды для логарифмических членов

        # Параметры для расчета EMA
        self.N = 5                     # Количество периодов для EMA
        self.alpha = 2 / (self.N + 1)  # Сглаживающий фактор
        self.EMA_prev = None           # Предыдущее значение EMA
        ...
        # Параметры времени
        self.sampling_interval = 0.1  # Интервал выборки в секундах
        self.total_duration = 10      # Общая продолжительность в секундах
